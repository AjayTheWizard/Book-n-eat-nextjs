import { Box, Heading, Text, useColorModeValue, Flex, Badge, Button } from "@chakra-ui/react";
import { ArrowForwardIcon } from "@chakra-ui/icons"
import { useRouter } from "next/router"

export const MenuCard = (props: { link: string }) => {
  const router = useRouter();
  const pushLink = ()=>{
    router.push("/restaurant/" + props.link)
  }
  return (
    <Box
      sx={{
    ...
    >
      ...
      <Text noOfLines={2}>
        ...
        soluta accusamus sapiente porro excepturi corrupti sint delectus
        doloremque repellat. Beatae eligendi suscipit corporis adipisci at.
      </Text>
      <Button onClick={pushLink} mt={2} rightIcon={<ArrowForwardIcon/>} colorScheme={useColorModeValue("red", "gray")}>View More</Button>
    </Box>
  );
};
